...
__global__ void Convolution_2D_Kernel(float *In,float *Out,
                                ...
                                const float * __restrict__ M)
{
    ...
    for(k=0;k<3;k++)
    {
        ...
        {
            Ns[ty][tx][k]=In[(row_i*width+col_i)*channels+k];
        }
        else
            Ns[ty][tx][k]=0.0f;
        __syncthreads();
        ...
        if((ty<O_TILE_WIDTH)&&(tx<O_TILE_WIDTH))
        {
            ...
            for(i=0;i<Mask_width;i++)
            {
                ...
                {
                    output+=M[i*Mask_width+j]*Ns[i+ty][j+tx][k];
                }
            }
        ...
        {
                Out[(row_o*width+col_o)*channels+k]=clamp(output,0,1);
        }     
        __syncthreads();
    }
}
